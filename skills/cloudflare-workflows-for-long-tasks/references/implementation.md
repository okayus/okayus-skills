...
```jsonc
{
  "compilerOptions": {
    ...
    "types": ["@cloudflare/workers-types"], // ← Workflow / Ai / D1Database etc visible here
    "noEmit": true
  },
  "include": ["worker"]
}
...
The `"types": ["@cloudflare/workers-types"]` line is what makes `Workflow<Params>` resolve as a global without an import. If you remove it, you'll get the misleading "no exported member 'Workflow'" error on the binding line and try to fix it with an import that doesn't work.
