...
Arming the opt-in needs *Allow auto-merge* in repo settings (`gh repo edit --enable-auto-merge`; public + Free is enough) and required checks on the base branch — GitHub offers auto-merge only when the PR cannot merge immediately. Write the exception list into `CLAUDE.md` (matatabetai: `drizzle/`, `.github/**`, `.claude/**`, `docs/adr/**` PRs wait for the human). UNVERIFIED: the fine-grained-PAT + `enablePullRequestAutoMerge` combination (first armed PR will say).
