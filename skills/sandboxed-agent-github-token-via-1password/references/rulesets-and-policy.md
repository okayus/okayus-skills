...
Arming the opt-in needs *Allow auto-merge* in repo settings (`gh repo edit --enable-auto-merge`; public + Free is enough) and required checks on the base branch — GitHub offers auto-merge only when the PR cannot merge immediately — and **with no ruleset, `gh pr merge --auto` never waits for CI**: a PR GitHub already reports mergeable is merged on the spot (`autoMergeRequest: null`), and one whose mergeability is still `UNKNOWN` gets `Protected branch rules not configured for this branch (enablePullRequestAutoMerge)` (verified 2026-08-29, matatabetai #13/#14, okayus-skills #28). Create the ruleset first. Write the exception list into `CLAUDE.md` (matatabetai: `drizzle/`, `.github/**`, `.claude/**`, `docs/adr/**` PRs wait for the human). UNVERIFIED: the fine-grained-PAT + `enablePullRequestAutoMerge` combination (first armed PR will say).
