...
- ~~UNVERIFIED~~ **Verified 2026-08-30 in matatabetai**: `gh pr merge --auto --squash` from a `./shell.sh` shell (fine-grained PAT, Contents + Pull requests) arms auto-merge through the GraphQL `enablePullRequestAutoMerge` mutation — PR #17 shows `autoMergeRequest.enabledAt` 34 s before `mergedAt`, i.e. GitHub waited for `ci` and merged. The first attempt (#16) proved nothing because `ci` was already green when the command ran and gh took the immediate-merge path — arm right after `gh pr create`.
