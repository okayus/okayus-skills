...
description: Let a sandboxed coding agent (Claude Code in the docker sandbox) push branches and open PRs itself with a GitHub fine-grained PAT scoped to ONE repository, resolved from 1Password into the shell that runs it (`op run --env-file -- docker exec -e GH_TOKEN …`, wrapped in `./shell.sh`). Use when in-container pushes fail with `401` or `gh` says it is not logged in; when a plain `docker compose up -d` recreated the container and killed the running agent session; when `docker inspect` shows a bare `GH_TOKEN` with no `=` in `Config.Env`; when the agent needs `gh pr create` / `gh pr checks` directly; or when choosing between this and sandboxed-agent-git-relay because a repo-scoped, expiring credential inside the boundary is an acceptable trade. Covers why it must be your own PAT, the permission set (no Workflows), why the token enters at `docker exec` time rather than the compose `environment:`, the allow/deny rules that replace the blanket `git push` deny, the rulesets that stay the real boundary, merge policy, rotation, and the service-account variant.
license: MIT
compatibility: Designed for Claude Code and similar agents. Host = Linux/macOS with the Docker Compose plugin (v2 or later; behaviour here verified on v5.3.1 / Engine 29.6.2), 1Password CLI (`op`) and a 1Password account; the claude-code-docker-sandbox layout (`.docker/`, `docker-compose.yml` + gitignored override, bind-mounted repo sharing `.git` with the host); the container image already has git + gh. Repo on GitHub owned by you (personal account, or an org you are a member of) — fine-grained PATs can't be used by outside collaborators.
metadata:
  author: okayus
  version: "0.2.0"
---
...
- [ ] `./up.sh` is a plain `docker compose up -d` — **no credential, idempotent, safe to repeat**; `./shell.sh` wraps `op run --env-file=.docker/sandbox.env -- docker exec -it -e GH_TOKEN <container> "$@"` and fails loudly when the container is down or `op` resolved nothing
...
host  ./up.sh    =  docker compose up -d              ← no credential; idempotent, safe to repeat
      ./shell.sh =  op run --env-file=.docker/sandbox.env -- docker exec -it -e GH_TOKEN <container> "$@"
           │        (1Password unlock; GH_TOKEN="op://<vault>/<item>/credential" → real value)
           │        -e GH_TOKEN is NAME-ONLY: value comes from op run's env, never from argv (`ps`-safe)
           └─ no op session / nothing resolved → shell.sh exits before exec: no token, no push
...
| `op run` + `docker exec -e` | token → one shell's env, once per `./shell.sh` | put the value in argv, in a file, or in the container's config |
...
3. **Env template + the two wrappers**: `.docker/sandbox.env` = `GH_TOKEN="op://<vault>/github-pat-<repo>-sandbox/credential"` (gitignored — it is a reference, but host-specific); `up.sh` = `exec docker compose up -d "$@"` (no `op`: starting the container has nothing to do with the token); `shell.sh` = `op run --env-file=.docker/sandbox.env -- docker exec -it -e GH_TOKEN <container> "$@"`, guarded so it fails loudly when the container is down or `op` resolved nothing. Copy-ready in [references/compose-and-git-wiring.md](references/compose-and-git-wiring.md).
...
- **`op run … -- env` prints `<concealed by 1Password>` — exactly 24 characters.** 1Password masks output matching a secret it injected, so a length check routed through `env` reads 24 for a 93-character PAT and looks like a truncated or broken token. It is the mask, not the value. Measure with output that can't match the secret: `op run --env-file=.docker/sandbox.env -- sh -c 'echo ${#GH_TOKEN}'`.
...
| 0.1.x claim | Verdict | How it was checked |
|---|---|---|
| Compose drops a valueless `environment` key it can't resolve | **false** | `Config.Env` keeps a bare `GH_TOKEN` with no `=`; `docker compose config` prints `GH_TOKEN: null` |
| …but the container is still tokenless | true | `env` inside prints no `GH_TOKEN` line — fail-closed survives the correction |
| `cut -d= -f1 \| grep -c` tells you whether the token arrived | **false** | counts the bare key as present; `grep -c '^GH_TOKEN=.'` is the check that works |
...
| `op run -- env` shows the token's real length | **false** | masked to `<concealed by 1Password>` = exactly 24 chars, for a 93-char PAT |
