...
- **Watch-path excludes verified.** The next merge after the deploy.yml one touched only `CLAUDE.md`, `docs/adr/002-*.md`, `docs/log.md`, `docs/plans/host-setup.md`, `docs/roadmap.md`, `docs/status.md`. Two minutes later the commit still carried **only `ci`** and `/health` still served the previous deploy — so the excludes matched every one of those nested paths, and a skip is *silent*: no check-run is created.
